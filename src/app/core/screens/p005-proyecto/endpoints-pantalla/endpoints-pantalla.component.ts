...
import { EndpointPantalla } from 'src/app/core/models/endpoint.model';
...
export class EndpointsPantallaComponent {
  ...
  async crearCarpeta(){
    if (this.carpetaForm.valid){
      await this.carpetaService.crearCarpetaPantalla(
  ...
  async pedirEndpoints(){
    const endpoints = await this.endpointService.obtenerPantallasProyecto(this.idProyecto) as EndpointPantalla[]
    endpoints.forEach(e => {
